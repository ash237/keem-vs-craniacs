...
class Character extends FlxSprite
{
	...
	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false)
	{
		...
		switch (curCharacter)
		{
			...
			case 'gf-car':
				...
				playAnim('danceRight');
			case 'gf-draco':
				frames = Paths.getSparrowAtlas("Draco");
				animation.addByIndices('danceLeft', 'Drac', [0, 1, 2, 3, 4], "", 24, false);
				animation.addByIndices('danceRight', 'Drac', [5, 6, 7, 8, 9], "", 24, false);
			...
			case 'keem':
				// DAD ANIMATION LOADING CODE
				tex = Paths.getSparrowAtlas("keem");
				frames = tex;
				animation.addByPrefix('idle', 'KeemstarIdle', 24, false);
				animation.addByPrefix('singUP', 'Keemstarup', 24);
				animation.addByPrefix('singRIGHT', 'Keemstarright', 24);
				animation.addByPrefix('singDOWN', 'Keemstardown', 24);
				animation.addByPrefix('singLEFT', 'Keemstarleft', 24);
				animation.addByPrefix('scream', 'KeemstarSCREAM', 24);
	
				addOffset('idle');
				addOffset("singUP", -39, 16);
				addOffset("singRIGHT", -31, -25);
				addOffset("singLEFT", 29, -13);
				addOffset("singDOWN", -122, -132);
				addOffset("scream", -122, 138);
	...
	override function update(elapsed:Float)
	{
		if (!isPlayer)
		{
			if (animation.curAnim.name.startsWith('sing') || animation.curAnim.name.startsWith('scream'))
			{
				holdTimer += elapsed;
			}
	...
	public function dance(?ignoreDebug:Bool = false)
	{
		if (!debugMode || ignoreDebug)
		{
			switch (curCharacter)
			{
				case 'gf' | 'gf-car' | 'gf-christmas' | 'gf-pixel' | 'gf-draco':
