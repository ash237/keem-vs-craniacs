...
class PlayState extends MusicBeatState
{
	...
	var AAAAAAAAAAAAAA:Bool = false;
	var AAAAAAAAAAAAAAtimer:Int = 0;

	//var forcedCamPos:FlxPoint;
	...
	override public function create()
	{
		...
		else if (stageCheck == 'deadDollar')
		{
			camZooming = true;
			defaultCamZoom = 0.3;
			curStage = 'deadDollar';
	
			var bg:FlxSprite = new FlxSprite(-900, -450).loadGraphic(Paths.image('349'));
			add(bg);
		}
		else
		{
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				if (unspawnNotes.length > 0)
					...
					oldNote = null;
				var screaming = songNotes[3];
				var swagNote:Note = new Note(daStrumTime, daNoteData, false, oldNote, false, screaming);
				...
				for (susNote in 0...Math.floor(susLength))
				{
					...
					var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + Conductor.stepCrochet, daNoteData, false, oldNote, true, screaming);
		...
		for (i in unspawnNotes) {
			if (i.scream) {
				for (e in unspawnNotes) {
					if (e.strumTime == i.strumTime)
						e.scream = true;
				}
			}
		}
